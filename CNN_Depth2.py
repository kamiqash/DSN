...
def Depth2_CNN(loopidx,CTU_FLAG,MOTION_FLAG,CTU_NN_FLAG,rate_list,DBlist,PRINT_STATUS,PRINT_STATS,TestInfo,EPOCHS,outputFolder,MULTI_CTU_FLAG,METHODOLOGY,PreDefinedTest,DEBUG):
    ...
    for DB in DBlist:
        ...
        for rate in rate_list:
            Record_Rates = []
            if (PRINT_STATUS):
                print("Depth 2: Processing Rate: %s, FLAGS CTU_FLAG; %d, CTU_NN_FLAG: %d , TEST: %s" % (
                rate, CTU_FLAG, CTU_NN_FLAG, TestInfo))

            IMAGE_SIZE = (16, 16)
            batch_size = 32

            pathDrive = outputFolder+'Depth2/QP_' + str(rate) + '/'
            ...
            Record=LoadFiftySixViewsDepth2(TotLF, ctuPath, metaPath, rate, PRINT_STATUS,METHODOLOGY,DB)
    ...
    # FROM HERE
            ...
            if (CTU_NN_FLAG):  # CTU + Feature vector case
                ...
                img_input_shape= (16,16,5) # The input CTU is 64x64
                z,InpArray=Basic_NN_CTU_D0(img_input_shape) # Basic NN CTU only case
