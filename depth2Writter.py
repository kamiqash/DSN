# This Function is called by function main_Predictor and gets model and test LF images need to be evaluated using ML scheme for Depth 2.
...
def writeDepth2TestPrediction(model,Test_df,Test,rate, depthValue, TestImage,outputDNNFolder,methodology,CTU_NN_FLAG,CTU_FLAG,CTU_NN_Rate_Prev):
    ...
    for ImgIdx in range(0, len(NameofTestImage)):
        ...
        image_arr3 = tf.image.resize(image_arr3, (16, 16)).numpy()
    ...
    with open(filename2Store, 'w') as f:
        for pIndex in range(0, len(NameofTestImage)):
            strW = "%d %d 2 %d %d %d 9 9 9 9 %d\n" % (int(VN[pIndex]), int(CTU_L[pIndex]), int(PartNo[pIndex]), int(PartNo2[pIndex]), int(PU_L[pIndex]),
            p[pIndex])

            f.write(strW)
            npList = []
        f.close()
        print("File writing at Depth 2, Test image %s completed", TestImage)