...
# Flags that define different variant of the code
...
DEBUG=1 # This flag enable debugging. Total LF are 4 and total test images are 1.
        ...
        for FLAG_indx in range(0,len(CTU_FLAG_HV)): # Loop over flags
            ...
            for loopidx in range (0,len(Test)):# Loop over Test A to C
                ...
                if(depth=='0'): # Depth 0 work
                    print('Depth0')
                    Depth0_CNN(loopidx,CTU_FLAG,MOTION_FLAG,CTU_NN_FLAG,rate_list,DBlist,PRINT_STATUS,PRINT_STATS,Test[loopidx],EPOCHS,outputFolder,MULTI_CTU_FLAG,METHODOLOGY,PreDefinedTest,DEBUG)
                elif(depth=='1'):# Depth 1 work
                    print('Depth1')
                    Depth1_CNN(loopidx,CTU_FLAG,MOTION_FLAG,CTU_NN_FLAG,rate_list,DBlist,PRINT_STATUS,PRINT_STATS,Test[loopidx],EPOCHS,outputFolder,MULTI_CTU_FLAG,METHODOLOGY,PreDefinedTest,DEBUG)
                else:# Depth 2 work
                     print('Depth 2')
                     Depth2_CNN(loopidx, CTU_FLAG, MOTION_FLAG, CTU_NN_FLAG, rate_list,DBlist, PRINT_STATUS, PRINT_STATS,Test[loopidx], EPOCHS,outputFolder,MULTI_CTU_FLAG,METHODOLOGY,PreDefinedTest,DEBUG)
